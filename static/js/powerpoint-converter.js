class PowerPointConverter {
    constructor() {
        ...
        this.pdfToPptOptions = document.getElementById('pdfToPptOptions');
        this.extractImages = document.getElementById('extractImages');
        this.preserveLayout = document.getElementById('preserveLayout');
        this.createAnimations = document.getElementById('createAnimations');
    ...
    handleFileSelect(file) {
        ...
        if ((isPDF && conversionType === 'pdfToPpt') || (isPPT && conversionType === 'pptToPdf')) {
    ...
    updateUI() {
        ...
        // 파일 형식 제한 설정
        if (conversionType === 'pdfToPpt') {
            this.fileInput.accept = '.pdf';
            this.pdfToPptOptions.style.display = 'block';
        } else {
            this.fileInput.accept = '.pptx';
            this.pdfToPptOptions.style.display = 'none';
        }
    }
    ...
    async convertFile() {
        ...
        const conversionType = document.querySelector('input[name="conversionType"]:checked').value;
        const endpoint = conversionType === 'pdfToPpt' ? '/api/convert/pdf-to-ppt' : '/api/convert/ppt-to-pdf';
        ...
        // PDF to PPT 옵션 추가
        if (conversionType === 'pdfToPpt') {
            formData.append('extractImages', this.extractImages.checked);
            formData.append('preserveLayout', this.preserveLayout.checked);
            formData.append('createAnimations', this.createAnimations.checked);
        }
        ...
        try {
            ...
            this.downloadBtn.download = `${file.name.split('.')[0]}.${conversionType === 'pdfToPpt' ? 'pptx' : 'pdf'}`;
